...
const Details = () => {
                ...
                <div className='w-[85%] md:w-[80%] sm:w-[90%] lg:w-[90%] h-full mx-auto pb-16'>
                    <div className='grid grid-cols-2 md-lg:grid-cols-1 gap-8'>
                        <div>
                            ...
                            <img className='h-[600px] w-full object-contain' src={image ? image : product.images?.[0]} alt="" />
                            </div>
                            <div className='py-3'>
                                {
                                            ...
                                            product.images.map((img, i) => {
                                                return (
                                                    <div key={i} onClick={() => setImage(img)}>
                                                       <img className='h-[60px] w-full object-contain cursor-pointer' src={img} alt="" />
                                                    </div>
                                                )
                                            })
                                        }
                        ...
                        <div className='flex flex-col gap-5'>
                            ...
                            <span className='text-green-500 flex items-center'>vendido por 
  <img 
  ...
  /> <p>  {product.sellerName}</p>
                            ...
                            <div className='text-slate-600'>
                                 <p>
                                    {product.description ? product.description.split(' ').slice(0, 230).join(' ') : ''} 
                                    {product.description && product.description.split(' ').length > 230 && '...'}
                                </p>
                                </div>
                            ...
                            <div className='flex gap-3 pb-10 border-b'>
                                {
                                    product.stock ? <>
                                        ...
                                        <div>
                                            <button onClick={add_card} className='px-8 py-3 h-[50px] cursor-pointer hover:shadow-lg hover:shadow-purple-500/40 bg-purple-500 text-white'>Agregar al carrito</button>
                                        </div>
                                    </> : ''
                                }
                                <div>
                            ...
                            <div className='flex py-5 gap-5'>
                                ...
                                <div className='flex flex-col gap-5'>
                                    <span className={`text-${product.stock ? 'green' : 'red'}-500`}>
                                        {product.stock ? `En Stock(${product.stock})` : 'Fuera de stock'}
                                    </span>
                        ...
                        <div className='w-[72%] md-lg:w-full'>
                            <div className='pr-4 md-lg:pr-0'>
                                <div className='grid grid-cols-2'>
                                    ...
                                    <button onClick={() => setState('description')} className={`py-1 px-5 hover:text-white hover:bg-green-500 ${state === 'description' ? 'bg-green-500 text-white' : 'bg-slate-200 text-slate-700'} rounded-sm`}>Descripcion</button>
                                </div>
                                <div>
                                    {
                                        ...
                                        <SwiperSlide key={i}>
                                            <Link  className='block' to={`/product/details/${p.slug}`}>
                                                <div className='relative h-[250px]'>
                                                    ...
                                                    {
                                                        p.discount !== 0 && <div className='flex justify-center items-center absolute text-white w-[28px] h-[28px] rounded-full bg-red-500 font-semibold text-xs left-2 top-2'>{p.discount}%</div>
                                                    }
                                                </div>
